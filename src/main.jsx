...
function App() {
  ...
  async function login(e) {
    e?.preventDefault();
    if (!supabase) return alert('Supabase não configurado. Confira as variáveis VITE_SUPABASE_URL e VITE_SUPABASE_ANON_KEY na Vercel.');
  ...
  async function cadastrar() {
    ...
    await addLog('Nova revisão cadastrada', `${novo.codigo} ${novo.revisao} - ${obraSelecionada?.nome || ''}`, 'projetos', data.id);
  ...
  async function alterarStatus(id, status) {
    ...
    const { error } = await supabase.from('projetos').update({ status: statusBanco(status) }).eq('id', id);
  ...
  async function concluirTarefa(id) {
    const { error } = await supabase.from('tarefas').update({ status: 'Concluída' }).eq('id', id);
    if (error) return alert(error.message);
  ...
  const lista = useMemo(() => projects.filter(p =>
    ...
    [p.obra, p.setor, p.disciplina, p.codigo, p.titulo, p.revisao, p.status].join(' ').toLowerCase().includes(busca.toLowerCase())
    ...
    return (
      <div className="loginPage">
        <form className="loginCard" onSubmit={login}>
          <div className="brand"><Building2 /><h1>ObraCode Profissional</h1></div>
  ...
  return (
    <div className="app">
      <aside>
        <div className="brand side"><Building2 /><div><h2>ObraCode</h2><span>{user.perfil}</span></div></div>
        {menu.filter(m => can(user, m[0])).map(([id, nome, Icon]) => (
          <button key={id} className={tab === id ? 'active' : ''} onClick={() => setTab(id)}><Icon size={18} />{nome}</button>
        ))}
        <button className="logout" onClick={sair}><LogOut size={18} />Sair</button>
      </aside>

      <main>
        <header>
          <div><h1>{menu.find(m => m[0] === tab)?.[1]}</h1><p>Usuário: {user.email}</p></div>
          <div className="mobile"><Smartphone size={18} />Pronto para celular/PWA</div>
        </header>

        {loading && <div className="panel"><b>Carregando dados...</b></div>}
        ...
        {tab === 'dashboard' && <section>
          ...
          <div className="panel">
            ...
            <div className="flow"><span>Link Drive</span><b>→</b><span>Análise</span><b>→</b><span>Vigente</span><b>→</b><span>QR Code em campo</span><b>→</b><span>Obsoleto automático</span></div>
          </div>
        </section>}
        ...
        {tab === 'projetos' && <section>
          <div className="toolbar">
            <div className="search"><Search size={18} /><input placeholder="Buscar projeto, código, disciplina..." value={busca} onChange={e => setBusca(e.target.value)} /></div>
            <select value={obraFiltro} onChange={e => setObraFiltro(e.target.value)}>
              <option>Todas</option>
              {obras.map(o => <option key={o.id || o.nome}>{o.nome}</option>)}
            </select>
          </div>
          <div className="table"><table><thead><tr><th>Código</th><th>Título</th><th>Obra</th><th>Setor</th><th>Disciplina</th><th>Rev.</th><th>Status</th><th>QR</th><th>Ações</th></tr></thead><tbody>
            {lista.map(p => <tr key={p.id}>
              <td><b>{p.codigo}</b></td>
              <td>{p.titulo}<small>{p.arquivo}</small></td>
              <td>{p.obra}</td><td>{p.setor}</td><td>{p.disciplina}</td><td>{p.revisao}</td>
        ...
        {tab === 'upload' && <section>
          <div className="panel">
            ...
            <div className="formgrid">
              <select value={novo.obra_id} onChange={e => {
            ...
            <input className="fullInput" placeholder="Cole aqui o link compartilhado do PDF/DWG" value={novo.arquivo_url} onChange={e => setNovo({ ...novo, arquivo_url: e.target.value })} />
            <div className="drop"><Upload /><b>Arquivo externo via Drive/OneDrive</b><span>O Supabase guarda apenas o cadastro, link, QR Code, revisão e auditoria.</span></div>
            <button className="primary" onClick={cadastrar}><PlusCircle size={18} />Cadastrar revisão</button>
          </div>
        </section>}
        ...
        {tab === 'usuarios' && <section className="panel"><h3>Permissões por perfil</h3><table><thead><tr><th>Perfil</th><th>Acessos</th></tr></thead><tbody>{Object.entries(perfis).map(([p, mods]) => <tr key={p}><td><b>{p}</b></td><td>{mods.join(', ')}</td></tr>)}</tbody></table><p><Shield size={16}/> Os usuários reais são criados em Supabase → Authentication → Users. O perfil fica na tabela public.profiles.</p></section>}
